...
func registerOpenVPNEndpoints(registry *endpoint.Registry) {
	endpoint.RegisterUnsupported[option.OpenVPNClientEndpointOptions](registry, C.TypeOpenVPNClient, func(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.OpenVPNClientEndpointOptions) (adapter.Endpoint, error) {
		if !options.System {
	...
	endpoint.RegisterUnsupported[option.OpenVPNServerEndpointOptions](registry, C.TypeOpenVPNServer, func(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.OpenVPNServerEndpointOptions) (adapter.Endpoint, error) {
		if !options.System {
			return nil, E.New(`OpenVPN is not included in this build, rebuild with -tags with_openvpn,with_gvisor for system:false`)
		}
		return nil, E.New(`OpenVPN is not included in this build, rebuild with -tags with_openvpn`)
