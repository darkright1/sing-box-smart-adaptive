...
type IWANEndpointOptions struct {
	...
	// ServerSocketReaders enables the Linux SO_REUSEPORT ingress reader set.
	// Zero keeps the portable single-reader default; values above the
	// implementation cap are bounded to avoid accidental thread explosions.
	ServerSocketReaders uint8                          `json:"server_socket_readers,omitempty"`
